...
class Yacona {
    ...
    constructor( path, appName ){
        ...
        this.documents = {
            load: ( name ) => { return documents.load( utility.fixPath( paths.documents, this.name ), name ) },
            save: ( name, value ) => { return documents.save( utility.fixPath( paths.documents, this.name ), name, value ) }
        }
        this.config = {
            load: ( name ) => { return config.load( utility.fixPath( paths.appData, this.name ), name ) },
