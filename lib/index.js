...
class Yacona {
    ...
    on( eventName, fn ){
        if( ( eventName !== undefined && type( eventName ) === 'string' ) && ( fn !== undefined && type( fn ) === 'function' ) ){
            globalFunctions[this.name + '.' + eventName] = fn
            return true
        } else return status.error( this.name, 'MISSING_REQUIRED_ARGUMENTS' )
    }
    
    emit( options, data ){
        let eventName = options['app'] + '.' + options['event']
        if( globalFunctions[eventName] === undefined ) return status.error( this.name, 'EVENT_NOT_FOUND' )
        else return globalFunctions[eventName]( data )
    }
